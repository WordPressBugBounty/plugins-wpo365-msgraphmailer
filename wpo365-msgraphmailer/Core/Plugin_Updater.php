    ...
    public function check_update($_transient_data)
    {
        ...
        if (false !== $current && is_object($current) && isset($current->new_version)) {
            ...
            if (version_compare($this->version, $current->new_version, '<')) {
                ...
                $plugins = \get_plugins();
                $plugin = isset($plugins[$this->name]) ? $plugins[$this->name] : null;
                $version = !empty($plugin) ? $plugin['Version'] : null;

                if (!empty($version) && version_compare($version, $current->new_version, '<')) {
                    $_transient_data->response[$this->name] = $current;
                } else {
                    // The cache must be stale - let's delete it
